...
/*   Updated: 2024/01/03 00:07:10 by tlouro-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				int struct_i, int *string_i)
{
	...
	while (s[*string_i] == '\4' || s[*string_i] == '<' || s[*string_i] == '>')
		(*string_i)++;
	file_or_delimiter = mod_strdup(&s[*string_i], "\4<>");
...
char	**split_args(char *cmd, t_enviroment *enviroment, int struct_i)
{
	...
	while (cmd[i] != '\0')
	{
		if (cmd[i] == '>' || cmd[i] == '<')
		{
			if (manage_redirections(&cmd[i], enviroment, struct_i, &i) == -1)
				return (NULL);
		}
		else if ((i == 0 && !ft_isinstr("\4<>", cmd[i]))
			|| (!ft_isinstr("\4<>", cmd[i]) && ft_isinstr("\4<>", cmd[i - 1])))
		{
			enviroment->cmd[struct_i].args[j] = mod_strdup(&cmd[i], "\4<>");
