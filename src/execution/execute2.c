...
/*   Updated: 2024/01/23 16:10:37 by tlouro-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	run_builtin(t_cmd *cmd, t_enviroment *enviroment, t_pipe *pipes)
{
	...
	else if (ft_strcmp(cmd->args[0], "minishell") && enviroment->num_cmd == 1)
			...
			t_pipe *pipes)
{
	...
	if (cmd->delimiter->begin)
	{
		status = read_here_doc(cmd->delimiter, pipes->input_pipe[WRITE_END],
				enviroment, cmd->prio_in);
		if (status == -1)
			error_and_close_pipes(enviroment, pipes);
		else if (status == -2)
			return (5);
	}
	if (cmd->prio_in == IN && cmd->input_file->begin)
	{
